...
process_stats <- function(data, pod_spec_percent) {
  data %>%
    ...
    dplyr::mutate(
      pod_spec_percent = pod_spec_percent,
      spec_min = weight_target_value - weight_target_value * pod_spec_percent / 100,
      spec_max = weight_target_value + weight_target_value * pod_spec_percent / 100,
      ...
      # weight_MR = dplyr::if_else(
      #   !is.na(abs(weight_value - dplyr::lag(weight_value))),
      #   abs(weight_value - dplyr::lag(weight_value)), 0),
      weight_MR = abs(weight_value - dplyr::lag(weight_value)),
      weight_MR_mean = mean(weight_MR, na.rm = TRUE),
      # to be corrected by -1
